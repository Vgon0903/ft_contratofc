SELECT
       ...
       contrato.userid,  
       sum(rtporto.pesodescarregamento)pesodescarregamento,
       sum(rtporto.pesoretencao)pesoretencao,
...
left join (
select nfitem.contrato,nfitem.estab,retenporto.pesodescarregamento,retenporto.pesoretencao from nfcab
...
inner join retenporto on  retenporto.estab=nfitem.estab
                    ...
                    WHERE  NOT exists (SELECT * FROM NFITEMAPARTIRDE
                                       ...
                                       NFCFGDV.NOTACONF = NFDV.NOTACONF
                                        AND NFCFGDV.NOTACONF IN (271,276)
...
GROUP BY CONTRATO.ESTAB,CONTRATO.CONTRATO)VLRREC

     on contrato.estab=VLRREC.estab
    and contrato.contrato=VLRREC.contrato
            
                  
where contrato.estab IN (12,25,26,30,31,34)
  

 and contratoite.item IN(1,3,2)
